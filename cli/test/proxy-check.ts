 ...
 *
 * Usage:  bun run cli:test:proxy
 *
 * Deliberately not named `*.test.ts`: it is a standalone vite-node script, and
 * the web suite (`npm test`) runs vitest in a browser environment where these
 * Node-only checks cannot run.
 *
 * Prerequisites:
