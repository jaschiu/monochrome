...
/**
 * Write to stderr directly rather than via `console`, so logging keeps working
 * while shared web modules' console output is temporarily intercepted.
 */
...
export const log = {
    ...
    warn(...args: unknown[]) {
        clearProgress();
        writeLine('\x1b[33m⚠\x1b[0m', ...args);
    },

    error(...args: unknown[]) {
        clearProgress();
        writeLine('\x1b[31m✖\x1b[0m', ...args);
    },

    success(...args: unknown[]) {
        clearProgress();
        writeLine('\x1b[32m✔\x1b[0m', ...args);
    },

    verbose(...args: unknown[]) {
        if (verboseEnabled) {
            clearProgress();
            writeLine('\x1b[90m', ...args, '\x1b[0m');
        }
    },
