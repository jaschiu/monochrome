...
export interface TrackMetadata {
    ...
    tidalTrackUrl: string | null;
    tidalAlbumUrl: string | null;
    albumReleaseDate: string | null;
    /** Serialized track object, matching `TIDAL_DATA` in `js/metadata.js`. */
    tidalData: string | null;
...
function buildMetadataArgs(metadata: TrackMetadata, extension: string, writeArtistsSeparately: boolean): string[] {
    ...
    // Same custom tag set `addMetadataToAudio` writes in `js/metadata.js`, so
    // CLI downloads expose the same fields to `readTrackMetadata` (which parses
    // and filters TIDAL_DATA) when they are imported as local files.
    meta('TIDAL_TRACK_ID', metadata.tidalTrackId);
    meta('TIDAL_ALBUM_ID', metadata.tidalAlbumId);
    meta('TIDAL_TRACK_URL', metadata.tidalTrackUrl);
    meta('TIDAL_ALBUM_URL', metadata.tidalAlbumUrl);
    meta('ALBUM_RELEASE_DATE', metadata.albumReleaseDate);
    meta('TIDAL_DATA', metadata.tidalData);
...
export function buildTrackMetadata(track: Track, album: AlbumSummary | null = null, rgInfo: RgInfo | null = null): TrackMetadata {
    ...
    // `js/metadata.js` serializes the whole track (CRLF-normalised) into
    // TIDAL_DATA; keep the shape identical so the reader's filter finds the
    // fields it expects.
    const tidalData = JSON.stringify({ ...track, album: albumData }, null, 2).replace(/\n/g, '\r\n');
    ...
    return {
        ...
        tidalTrackUrl: track.url?.trim() || null,
        tidalAlbumUrl: albumData.url?.trim() || null,
        albumReleaseDate: albumData.releaseDate?.trim() || null,
        tidalData,
        replayGain: rgInfo || null,
    };
}
